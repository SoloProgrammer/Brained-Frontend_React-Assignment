...
function Form(props) {
  ...
  const { submit_txt, curdFunc, notedetail, id } = props
  ...
  const HandleSubmit = (e) => {
    ...
    if (curdFunc === "addnote") {
      ...
      addnote(  note.title, note.description, note.tag,setNote)
      console.log(note)
    }
    else {
  ...
  return (
    ...
    <form className='my-3' onSubmit={HandleSubmit}>
      ...
      <div className="form-group my-3">
        ...
        <textarea rows={5} value={note.description} required type="text" className="form-control" id="description" name='description' onChange={Onchange} placeholder="Description Here...." />
      </div>
      <div className="form-group my-3">
      ...
      <button disabled={note.title.length < 1 || note.tag.length < 1 || note.description.length < 1 ? true : false} type="submit" className="btn btn-primary" ><i className="fa-solid fa-note-sticky white"></i> {`${submit_txt}`}</button>
    </form>
